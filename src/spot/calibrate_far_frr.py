...
1) Face identities file (default):
   data/models/face_identities.json
...
Usage examples:
    ...
    python main.py calibrate --dataset data/models/face_identities.json --output data/models/threshold_calibration.json
    python main.py calibrate --dataset my_embeddings.json --apply
"""
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=model_path("threshold_calibration.json"),
        help="Path to write calibration summary JSON.",
    )
    parser.add_argument(
        "--apply",
        action="store_true",
        help="Apply per-label thresholds to the shared face_identities.json file when labels match.",
    )
